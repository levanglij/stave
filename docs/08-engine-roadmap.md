...
| § 6 SRFP marketplace + tranches | 30% | Single-class on-chain MVP shipped: 5 Anchor instructions (`create_work`, `list_shares`, `buy_shares`, `deposit_royalty`, `claim_royalty`), 15/15 passing local tests, devnet deploy queued. Investor UI live at stave.cc with 8 listings + 4 thematic indices + per-catalog returns calculator + real Phantom/Solflare wallet flow. Tranche structuring (senior/mezz/growth) and monthly distribution reporting still missing. |
