...
Roughly **5–8% of the full architecture is implemented**. That is consistent with the architecture's own phasing (§ 9 budgets 9 months for Phase 1 alone). The hackathon submission ships a credible MVP slice; this document tracks what closes the gap from MVP to a real institutional product.
...
| § 6 SRFP marketplace + tranches | 0% | No on-chain program, no tranche structuring, no investor UI, no monthly distribution reporting |
...
| § 9 GTM Phase 1 targets | ~5% | Demo engine on synthetic data; no PRO integration, no 50–100 catalog backtest, no classifier, no ±15% MAE validation |
